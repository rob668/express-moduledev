    ...
    Run(config) {
		...
		app.use(function (req, res, next) {
		    ...
		    res.render = function (view, options, callback) {
		        ...
		        //表示不是顶级的
		        if (req.originalUrl !== "/") {
					let moduleObject = getModules(req.baseUrl.substr(1))
					if( moduleObject != null ){
						if( moduleObject.subModules ){
							moduleName =  moduleObject.subModules
						}else{
							moduleName = moduleObject.prefix.substr(1)
						}
					}else{
						moduleName = req.baseUrl.split('/')[1];
					}
				}

...
exports.init = function(app){
    ...
    var APP = {
        "ctrls":{
            get : function(ctrlName, m){
                let _k = ctrlName,
                _ctrlPATH = "ctrls/"+ ctrlName;

                if( m !== undefined && m !== ""){
                    _k = m +"-"+ ctrlName;
                    _ctrlPATH = "modules/"+m+"/"+ _ctrlPATH;
                }

                let __c = _CTRLS[_k];
                if( _CTRLS[_k] == undefined ){
                    //先检查文件是否存在
                    if( false === utils.fsExistsSync(process.cwd()+'/app/' + _ctrlPATH +'.js') ){
                        throw new Error('指定的控制器'+ ctrlName +'不存在'+ _ctrlPATH);
                    }
                    __c = require(process.cwd()+'/app/' + _ctrlPATH);
